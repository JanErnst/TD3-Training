...
class TD3(object):
  ...
  def __init__(self, state_dim, action_dim, max_action):
    ...
    self.actor_target.load_state_dic(self.actor.state_dict())
    self.actor_optimizer = torch.optim.Adam(self.actor.parameters())
    self.critic = Critic(state_dim, action_dim).to(device)
    self.critic_target = Critic(state_dim, action_dim).to(device)
    self.critic_target.load_state_dic(self.critic.state_dict())
    self.critic_optimizer = torch.optim.Adam(self.critic.parameters())
    self.max_action = max_action

  def select_action(self, state):
    state = torch.Tensor(state.resape(1, -1)).to(device)
  ...
  def train(self, replay_buffer, iterations, batch_size = 100, discount = 0.99, tau = 0.005,  policy_noise = 0.2, noise_clip = 0.5, policy_freq = 2):
    ...
    for it in range(iterations):
      ...
      # Step 5: From the next state s', the Action target plays the next action a'
      next_action = self.actor_target(next_state)

      # Step 6:  add gaussian noise to this next action a' and  clamo it in a range of values supported by the environment
      ...
      #Step 9:  get the final targetr of the two ciritc models, which is: Qt = r + y + min(Qt1, Qt2), where y is the discount factor
      ...
      #Step 11:
      critic_loss = F.mse_loss(current_Q1, target Q) + F.mse_loss(current_Q2, target_Q)
      ...
      if it % policy_freq == 0:
        ...
        for param, target_param in zip(self.actor.parameters(), self.actor_target.parameters():
          param_target.data.copy_(tau * param.data + (1 - tau) * target_param.data) 

        #Step 15:
        for param, target_param in zip(self.cricit.parameters(), self.critic_target.parameters():
          param_target.data.copy_(tau * param.data + (1 - tau) * target_param.data) 
...
while total_timesteps < max_timesteps:
  ...
  if done:
    ...
    # If  are not at the very beginning,  start the training process of the model
